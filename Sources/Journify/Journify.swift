...
public class Journify {
    
    private static var sharedInstance: Journify?
    
    public static func shared() -> Journify {
        if sharedInstance == nil {
            sharedInstance = Journify(configuration: Configuration(writeKey: "Your_Key"))
        }
        return sharedInstance!
    }
    
    public static func setup(with configuration: Configuration) {
        sharedInstance = Journify(configuration: configuration)
    }
    ...
    public static var debugLogsEnabled = false {
        didSet {
            JournifyLog.loggingEnabled = debugLogsEnabled
        }
    }
...
extension Journify {
    ...
    /// Tells this instance of Analytics to flush any queued events up to Journify.com.  This command will also
    ...
    public func version() -> String {
        return Journify.version()
    }
    
    /// Retrieve the version of this library in use.
    /// - Returns: A string representing the version in "BREAKING.FEATURE.FIX" format.
    public static func version() -> String {
        return __journify_version
    }
}
...
extension Journify {
    ...
    public var hasUnsentEvents: Bool {
        if let journifyDest = self.find(pluginType: JournifyDestination.self) {
            if journifyDest.pendingUploads > 0 {
                return true
            }
            if journifyDest.eventCount > 0 {
                return true
            }
        }
