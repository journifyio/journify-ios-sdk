...
internal class JournifyLog: UtilityPlugin {
    public var filterKind = LogFilterKind.debug
    weak var analytics: Journify?
    ...
    internal static weak var sharedAnalytics: Journify? = nil
    
    #if DEBUG
    internal static var globalLogger: JournifyLog {
        get {
            let logger = JournifyLog()
            logger.addTargets()
            return logger
        }
    }
    ...
    func configure(analytics: Journify) {
        ...
        JournifyLog.sharedAnalytics = analytics
        addTargets()
    }
...
internal extension Journify {
    /// The internal logging method for capturing all general types of log messages related to Journify.
    /// - Parameters:
    ...
    static func journifyLog(message: String, kind: LogFilterKind? = nil, function: String = #function, line: Int = #line) {
        if let shared = JournifyLog.sharedAnalytics {
            shared.apply { plugin in
                if let loggerPlugin = plugin as? JournifyLog {
            ...
            JournifyLog.globalLogger.log(log, destination: .log)
            #endif
        }
    }
    
    /// The internal logging method for capturing metrics related to Journify or other libraries.
    /// - Parameters:
    ...
    static func journifyMetric(_ type: MetricType, name: String, value: Double, tags: [String]? = nil) {
        JournifyLog.sharedAnalytics?.apply { plugin in
