...
public protocol LogTarget {
    ...
    /// - important: Use the Journify Network stack for Journify library compatibility and simplicity.
...
/// The Journify logging system has three types of logs: log, metric and history. When adding a target that
...
extension Journify {
    ...
    /// The public logging method for capturing all general types of log messages related to Journify.
    /// - Parameters:
    ...
    public func log(message: String, kind: LogFilterKind? = nil, function: String = #function, line: Int = #line) {
        apply { plugin in
            ...
            if JournifyLog.loggingEnabled == false {
                return
            }
    ...
    /// The public logging method for capturing metrics related to Journify or other libraries.
    /// - Parameters:
    ...
    /// Used to track the history of events as the event data travels through the Journify Event Timeline. As
...
extension Journify {
    ...
    public func add(target: LogTarget, type: LoggingType) {
        apply { (potentialLogger) in
            if let logger = potentialLogger as? JournifyLog {
                do {
                    try logger.add(target: target, for: type)
                } catch {
                    Self.journifyLog(message: "Could not add target: \(error.localizedDescription)", kind: .error)
                }
            }
        }
    }
